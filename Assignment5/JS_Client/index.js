...
mqttClient.on('connect', () => {
    ...
    mqttClient.subscribe('gemma/temp', (err) => {
        if (err) {
            console.error('Failed to subscribe to MQTT topic:', err);
        } else {
            console.log('Subscribed to MQTT topic: gemma/temp');
        }
    });
});
...
mqttClient.on('message', async (topic, message) => {
    ...
    if (message[message.length-1] == 0)
        message_str = message.toString('utf-8', 0, message.length-1);
    else
        message_str = message.toString('utf-8', 0, message.length);
    const msg = JSON.parse(message_str).msg;
...
app.post('/', async (req, res) => {
    try {
        ...
        mqttClient.publish('gemma/temp', JSON.stringify(messageObject));
        res.sendStatus(200);
    } catch (error) {
